...
def bin_search(sorted_list, item_to_find):
    ...
    # Поиск будет выполняться пока min граница не будет равна max, т.е. искомое значение отсутствует
    while min_index <= max_index:
        ...
        # Если искомый элемент меньше серединного значению списка, то загаданное число находится слева,
        elif item_to_find < sorted_list[mid_index]:
            # Поэтому max граница теперь становится серединным значение -1 элемент, который мы уже проверили в первом if
            max_index = mid_index - 1
            # Если же искомый элемент больше серединного значению списка, то загаданное число находится справа,
        else:
            # Поэтому min граница теперь становится серединным значение +1 элемент, который мы уже проверили в первом if
